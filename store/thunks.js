...
const API_BASE_URL = 'https://memory-game-api-wki4.onrender.com';
...
export const createGame = (navigation) => async (dispatch) => {
	try {
		const response = await axios.post(`${API_BASE_URL}/game`);
		const game = response.data;
		dispatch(setGame(game));
		navigation.navigate('Lobby', { gameID: game.game_id });
	} catch (error) {
		console.log(error);
	}
};
...
export const getUsersThunk = (game_id) => {
	return async (dispatch) => {
		try {
			const response = await axios.get(
				`${API_BASE_URL}/game/${game_id}/users`
			);
